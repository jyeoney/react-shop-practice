...
const Nav = (): JSX.Element => {
  const getCartList = useRecoilValue<IProduct[]>(cartState);
  const getProductsList = useRecoilValue(productsList);
  ...
  const setLight = () => {
    $html?.classList.toggle(themeDark, false);
    $html?.setAttribute("themeColor", themeLight);
    localStorage.setItem("theme", themeLight);
  };

  const setDark = () => {
    $html?.classList.toggle(themeDark, true);
    $html?.setAttribute("themeColor", themeDark);
  ...
  useEffect(() => {
    const searchList = getProductsList.filter((product) =>
  ...
  //       <label htmlFor="side-menu" className="flex-none lg:hidden btn btn-square btn-ghost sm:w-auto">
  //         <svg
  ...
  //           </svg>
  //           <svg className="swap-on fill-blck w-7 h-7" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
  ...
  //                   <button className="text-left hover:text-base-content" onClick={() => goToDetail(product.id)}>
  //                     {product.title}
  //                   </button>
  //                 </li>
  //               );
  ...
  // );
  return (
    <div className="fixed z-10 w-full navbar shadow-lg bg-white dark:bg-[#191D24] text-neutral-content">
      <div className="flex w-full xl:container xl:m-auto">
        <label htmlFor="side-menu" className="flex-none lg:hidden btn btn-square btn-ghost sm:w-auto">
          <svg
            xmlns="http://www.w3.org/2000/svg"
          ...
          <div ref={searchRef}>
            <div className="form-control">
              <input
                type="text"
                placeholder="검색"
                className="sm:flex w-full input input-ghost focus:outline-0 rounded-none sm:rounded bg-gray-300 dark:bg-gray-600 !text-gray-800 dark:!text-white transition-all js-searchInput"
                onChange={(event) => {
                  setSearchText(event.target.value);
                }}
                onFocus={() => setSearchFocus(true)}
            ...
            {searchFocus && searchText && searchProductList.length > 0 && (
              <ul
                className={`absolute top-14 left-12 w-60 max-h-96 p-2 bg-gray-600 overflow-y-scroll ${
                  searchFocus ? "block" : "hidden"
                }`}
